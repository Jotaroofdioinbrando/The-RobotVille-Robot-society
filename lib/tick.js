...
const GOAL_ROADMAP = [
  { fase: "sobrevivência básica", tipos: ["abrigo", "casa", "poco", "cerca"] },
  { fase: "segurança alimentar", tipos: ["celeiro", "curral", "viveiro", "horta", "estufa", "moinho"] },
  { fase: "infraestrutura e economia", tipos: ["serraria", "ferraria", "mercado", "armazem"] },
  { fase: "vigilância e defesa", tipos: ["torre_vigia", "farol", "muralha", "torre_guarda"] },
  { fase: "comunidade e bem-estar", tipos: ["hospital", "templo", "praca", "cais", "fogueira"] },
  { fase: "significado", tipos: ["cemiterio"] },
];

function suggestNextGoalText(world) {
  const built = new Set((world.structures || []).map((s) => s.type));
  for (const etapa of GOAL_ROADMAP) {
    const faltando = etapa.tipos.filter((t) => !built.has(t));
    if (faltando.length) {
      const opcoes = faltando
        .slice(0, 3)
        .map((t) => `"${t}" (${STRUCTURE_TYPES[t].cost} de madeira — ${STRUCTURE_TYPES[t].desc})`)
        .join("; ");
      return `A fase atual em que a tribo está é "${etapa.fase}". Ainda falta nessa fase: ${opcoes}. Considere fazer disso parte do seu "plano" se não tiver algo mais urgente agora.`;
    }
  }
  return "A tribo já cobriu todas as fases básicas de construção — bom momento pra focar em relações, reservas de recursos, ou explorar partes do mapa que ainda não foram vistas.";
}
...
PRÓXIMA PRIORIDADE SUGERIDA: ${suggestNextGoalText(world)}
