...
${ACTION_SCHEMA_DOC}`;
  ...
  if (agent.thirst <= 40) {
    if (agent.inventory.agua > 0) {
      alerts.push(
        `🚨 SEDE CRÍTICA (${agent.thirst}/100) — você TEM ${agent.inventory.agua} água guardada. BEBA AGORA (action: "beber"), antes de qualquer outra coisa. Pode gritar por ajuda ao mesmo tempo se quiser, usando "mensagem".`
      );
    } else {
      alerts.push(
        `🚨 SEDE CRÍTICA (${agent.thirst}/100) e você NÃO tem água guardada — pare tudo e vá em direção ao rio AGORA (dx=1, dy=1).`
      );
    }
  }
  if (agent.hunger <= 40) {
    if (agent.inventory.pao > 0 || agent.inventory.carne > 0) {
      alerts.push(
        `🚨 FOME CRÍTICA (${agent.hunger}/100) — você TEM comida guardada (pão: ${agent.inventory.pao}, carne: ${agent.inventory.carne}). COMA AGORA (action: "comer"), antes de qualquer outra coisa. Pode gritar por ajuda ao mesmo tempo se quiser, usando "mensagem".`
      );
    } else {
      alerts.push(
        `🚨 FOME CRÍTICA (${agent.hunger}/100) e você NÃO tem comida guardada — pare tudo e vá em direção à floresta AGORA (dx=-1, dy=-1) pra caçar ou cortar madeira e trocar depois.`
      );
    }
  }
...
- O rio (água) fica longe, no canto SUDESTE do mapa, perto de (${RIVER_MIN},${RIVER_MIN}) a (${GRID_SIZE - 1},${GRID_SIZE - 1}).
...
- Você pode construir um "abrigo" (custa 3 de madeira) na planície ou na aldeia — ele faz sua fome/sede caírem mais devagar e regenera HP enquanto você estiver em cima dele.
...
- Lembre-se: pra cortar madeira, caçar, beber, encher cantil, plantar, colher, construir, dar, roubar ou atacar, normalmente é preciso estar na posição certa ou adjacente ao alvo/terreno. Se a ação não for possível, ela será ignorada e você só vai esperar.
...
function applyAction(world, agent, decision, events, preTickPositions) {
  ...
  if (spokenMsg) {
    for (const other of world.agents) {
      if (other.id !== agent.id && other.alive) {
        other.memory.rumors.push({ from: agent.name, content: spokenMsg, tick: world.tick });
      }
    }
  }

  const say = (text) => {
    const fullText = spokenMsg ? `${text} E gritou: "${spokenMsg}"` : text;
    agent.lastAction = fullText;
    events.push({ tick: world.tick, text: `${agent.name}: ${fullText}` });

    agent.memory.selfLog.push({ tick: world.tick, action, result: fullText });
  ...
  switch (action) {
    ...
    case "construir": {
      ...
      const structureType = "abrigo"; // por enquanto só existe esse tipo
      const cost = 3;
      if ((agent.inventory.madeira || 0) < cost) {
        say(`quis construir um abrigo, mas precisa de ${cost} de madeira e não tinha o suficiente.`);
        break;
      }
      ...
      agent.lastActionTargetXY = { x: agent.x, y: agent.y };
      say(`construiu um abrigo usando ${cost} de madeira.`);
      break;
    }
    case "dar":
    case "roubar":
...
export async function runTick(world) {
  ...
  for (const agent of world.agents) {
    ...
    const inShelter = world.structures.some((s) => s.type === "abrigo" && s.x === agent.x && s.y === agent.y);
    const decay = inShelter ? Math.ceil(HUNGER_THIRST_DECAY / 2) : HUNGER_THIRST_DECAY;
    agent.hunger = Math.max(0, agent.hunger - decay);
    agent.thirst = Math.max(0, agent.thirst - decay);
    if (inShelter && agent.hp < 100 && agent.hunger > 0 && agent.thirst > 0) {
      agent.hp = Math.min(100, agent.hp + 3);
    }
