...
async function decideAction(world, agent) {
  ...
  if (!apiKey) {
    return survivalFallback(agent, { thought: `(faltando ${cfg.apiKeyEnv} no ambiente)`, action: "esperar" });
  }
  try {
    ...
    return survivalFallback(agent, { thought: `(sinal perdido: ${String(e.message || e).slice(0, 80)})`, action: "esperar" });
  }
}
