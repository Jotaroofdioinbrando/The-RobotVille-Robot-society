...
import { callChatCompletion, parseAgentDecision } from "./llm";
...
function buildPrompt(world, agent, visibility) {
  ...
  const system = `Você é ${agent.name}, um agente autônomo vivendo em Robotville, uma vila que começou vazia perto de uma floresta distante (madeira) e um rio distante (água). Vocês trocaram sementes, pão e água no início. ${AGENT_CONFIG[agent.provider === "openai" ? "gpt" : agent.provider]?.persona || ""}
${EPISTEMIC_RULES}
${ACTION_SCHEMA_DOC}`;
...
async function decideAction(world, agent) {
  ...
  const cfg = AGENT_CONFIG[agent.provider === "openai" ? "gpt" : agent.provider];
  const apiKey = process.env[cfg.apiKeyEnv];
  if (!apiKey) {
    return { thought: `(faltando ${cfg.apiKeyEnv} no ambiente)`, action: "esperar" };
  }
  try {
    const raw = await callChatCompletion({ baseURL: cfg.baseURL, apiKey, model: cfg.model, system, user });
    return parseAgentDecision(raw);
  } catch (e) {
    return { thought: `(sinal perdido: ${String(e.message || e).slice(0, 80)})`, action: "esperar" };
  }
}
