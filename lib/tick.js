...
function stepSign(from, to) {
  if (to > from) return 1;
  if (to < from) return -1;
  return 0;
}

// Quando o agente escolhe uma ação que exige estar num lugar (floresta/rio) mas
// não está lá, em vez de só "desperdiçar" o ciclo, ele dá um passo automático
// na direção certa. Isso evita loop infinito de tentativa falha quando o
// modelo (ex: Groq) erra o cálculo de direção.
function autoMoveToward(agent, tx, ty, say, label) {
  const dx = stepSign(agent.x, tx);
  const dy = stepSign(agent.y, ty);
  agent.x = clampGrid(agent.x + dx);
  agent.y = clampGrid(agent.y + dy);
  say(`${label} moveu-se para (${agent.x},${agent.y}).`);
}

function applyAction(world, agent, decision, events) {
  ...
  switch (action) {
    ...
    case "cortar_madeira": {
      if (!isForest(agent.x, agent.y)) {
        autoMoveToward(agent, 1, 1, say, "queria cortar madeira mas não está na floresta —");
        break;
      }
    ...
    case "cacar": {
      if (!isForest(agent.x, agent.y)) {
        autoMoveToward(agent, 1, 1, say, "queria caçar mas não está na floresta —");
        break;
      }
      if (Math.random() < 0.45) {
    ...
    case "encher_cantil": {
      if (!isNearRiver(agent.x, agent.y)) {
        autoMoveToward(agent, 14, 14, say, "queria encher o cantil mas está longe do rio —");
        break;
      }
...
export async function runTick(world) {
  ...
  aliveAgents.forEach((agent, i) => applyAction(world, agent, decisions[i] || { action: "esperar" }, events));
