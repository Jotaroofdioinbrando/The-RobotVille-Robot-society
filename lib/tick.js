...
import { AGENT_CONFIG, EPISTEMIC_RULES, SOCIAL_DYNAMICS_RULES, ACTION_SCHEMA_DOC } from "./agents";
...
function buildPrompt(world, agent, visibility) {
  ...
  const knownAgentsText = Object.entries(agent.memory.knownAgents)
    .map(([id, k]) => {
      ...
      const trustNote = trust < 30 ? " ⚠️ baixa, cuidado" : trust >= 70 ? " (alta, confiável)" : "";
...
${ACTION_SCHEMA_DOC}`;
  ...
  const nearbyWithSupplies = visibleAgents.filter((o) => {
    ...
    return inv && ((inv.pao || 0) > 0 || (inv.agua || 0) > 0 || (inv.carne || 0) > 0);
  });
