...
const LONG_TERM_LIMIT = 20; // quantas notas de longo prazo cada agente guarda
...
${ACTION_SCHEMA_DOC}`;
  ...
  if (agent.hunger <= 40) {
    if (agent.inventory.pao > 0 || agent.inventory.carne > 0) {
      alerts.push(
        `🚨 FOME CRÍTICA (fome ${agent.hunger}/100, ou seja, você está quase sem energia) — você TEM comida guardada (pão: ${agent.inventory.pao}, carne: ${agent.inventory.carne}). COMA AGORA (action: "comer"), antes de qualquer outra coisa. Pode gritar por ajuda ao mesmo tempo se quiser, usando "mensagem".`
      );
    } else if (nearbyWithSupplies.length) {
      alerts.push(
...
- Você pode construir (action "construir", campo "estrutura"): "abrigo" (3 madeira, reduz gasto de fome/sede pela metade e regenera HP), "casa" (6 madeira, versão mais forte e permanente do abrigo), "cerca" (2 madeira, dificulta que roubem de você por perto), "fogueira" (2 madeira, ponto de encontro da tribo à noite).
...
- Se for NOITE e sua sobrevivência básica já estiver garantida, considere ir até a aldeia ou uma fogueira conhecida pra se reunir com outros agentes visíveis e usar "confraternizar" ou "falar".
...
function applyAction(world, agent, decision, events, preTickPositions) {
  ...
  switch (action) {
    case "mover": {
      ...
      if (steps === 3) {
        agent.hp = Math.max(0, agent.hp - 5);
        say(`correu 3 quadrados até (${agent.x},${agent.y}), gastando um pouco de energia (-5 HP).`);
        if (agent.hp === 0) {
          agent.alive = false;
          events.push({ tick: world.tick, text: `${agent.name} desmaiou de exaustão ao correr e caiu.` });
        }
      } else if (steps === 2) {
        say(`moveu-se 2 quadrados até (${agent.x},${agent.y}).`);
      } else {
    ...
    case "pescar": {
      ...
      if (Math.random() < 0.5) {
        ...
        say("pescou com sucesso e conseguiu peixe.");
      } else {
    ...
    case "confraternizar": {
      ...
      const onFogueira = (world.structures || []).some(
        (s) => s.type === "fogueira" && s.x === agent.x && s.y === agent.y
      );
      const gain = onFogueira ? 8 : 5;
      for (const companion of companions) {
        ...
        companion.memory.rumors.push({
          ...
          content: `${agent.name} passou um tempo bom confraternizando comigo${onFogueira ? " perto da fogueira" : ""}.`,
          tick: world.tick,
        });
      }
      agent.lastActionTargetXY = { x: agent.x, y: agent.y };
      say(`confraternizou com ${companions.map((c) => c.name).join(", ")}${onFogueira ? " perto da fogueira" : ""}.`);
      break;
    }
    case "dar":
    case "roubar":
    case "atacar": {
      ...
      if (action === "dar") {
        ...
        adjustTrust(target, agent.id, 15);
        ...
        const targetHasCerca = (world.structures || []).some(
          (s) => s.type === "cerca" && chebyshev(s.x, s.y, target.x, target.y) <= 1
        );
        const stealChance = targetHasCerca ? 0.2 : 0.5;
        if (Math.random() < stealChance) {
          ...
          say(`tentou roubar de ${target.name}, mas foi flagrado e falhou${targetHasCerca ? " (a cerca ajudou a proteger)" : ""}.`);
        }
...
export async function runTick(world) {
  ...
  for (const agent of world.agents) {
    ...
    const home = world.structures.find(
      (s) => STRUCTURE_TYPES[s.type]?.decayDivisor > 1 && s.x === agent.x && s.y === agent.y
    );
