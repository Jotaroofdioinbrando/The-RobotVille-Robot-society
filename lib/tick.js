...
function pendingBuildText(world, agent) {
  ...
  if (!cfg) return null;
  return `🏗️ Você tentou construir "${pb.estrutura}" no ciclo ${pb.tick} e não conseguiu (terreno errado ou madeira insuficiente) — ainda não terminou isso. Se ainda fizer sentido, ande até uma planície ou aldeia próxima (ou junte mais madeira, precisa de ${cfg.cost}) e tente "construir" de novo com "estrutura": "${pb.estrutura}". Se mudou de ideia, pode ignorar — mas decida isso conscientemente, não só esqueça.`;
}
