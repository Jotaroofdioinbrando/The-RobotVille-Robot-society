// Mistral, SambaNova e outros provedores compatíveis expõem a mesma forma de
// API (chat completions estilo OpenAI), então dá pra usar um único chamador genérico.
...
export async function callChatCompletion({ baseURL, apiKey, model, system, user, extraBody }) {
  ...
  if (!resp.ok) {
    ...
    const err = new Error(`${model} respondeu HTTP ${resp.status}: ${text.slice(0, 200)}`);
