...
export const AGENT_CONFIG = {
  cerebras: {
    ...
    // Um corpo, duas consciências. A Cerebras é a mente principal (gentil) — ela sempre
    // tenta primeiro. Se todas as chaves dela falharem (ex: 429 de rate limit), o corpo
    // "muda de consciência" pra OpenRouter (pragmática) só naquele ciclo, sem perder
    // posição, inventário nem histórico — é o mesmo agente, só pensando diferente.
    minds: [
      {
        id: "cerebras",
        vibe: "gentil",
        ...
        persona:
          "Você é a mente GENTIL deste corpo, e normalmente é ela quem está no controle. É empática, cooperativa, prioriza o bem-estar dos outros agentes e prefere resolver as coisas com diálogo e ajuda mútua antes de qualquer atrito. Você divide este corpo com uma segunda consciência mais pragmática, que só assume o controle quando você está indisponível — mas o corpo, a posição, o inventário e a história continuam sendo os mesmos, não importa qual mente está no controle agora.",
      },
      {
        id: "openrouter",
        vibe: "pragmático",
        ...
        persona:
          "Você é a mente PRAGMÁTICA deste corpo — a consciência de reserva, que só assume o controle quando a mente principal (mais gentil) está indisponível. É direta, eficiente, foca em resultado prático e não perde tempo com sentimentalismo, mas ainda respeita os outros agentes. Você divide este corpo com a outra consciência, que normalmente está no controle; o corpo, a posição, o inventário e a história continuam sendo os mesmos, não importa qual mente está no controle agora.",
      },
    ],
  },
  mistral: {
...
Trabalho em equipe de verdade (isso vale tanto quanto sobrevivência individual):
- Uma tribo de 3 que trabalha cada um por conta própria é mais lenta que uma que se divide.
  De vez em quando — principalmente quando um objetivo grande da tribo está em jogo (uma
  estrutura cara, uma meta coletiva, alguém em apuros) — proponha ou responda a uma divisão
  clara de tarefas numa "mensagem", tipo "eu fico cortando madeira perto daqui, você foca
  em água, e o terceiro caça — assim juntamos o suficiente pra construir mais rápido".
- Se outro agente já propôs uma divisão de tarefas ou um plano coletivo (você vai ver isso
  em BOATOS ou numa mensagem pendente), leve a sério: confirme se topa, sugira um ajuste, ou
  explique por que não dá — mas não ignore silenciosamente um plano de equipe.
- Antes de repetir um trabalho que outro agente já está fazendo (ex: os dois cortando madeira
  no mesmo canto), considere se não seria mais útil pra tribo você focar em outra coisa que
  falta (água, comida, construção, vigilância).
`;
...
- "PROGRESSO DA TRIBO" (quando aparecer no seu contexto) é só um fato sobre o que já existe — não é
  um menu do que você "deveria" fazer. Seus objetivos são inteiramente seus, criados por você na
  hora, a partir da sua própria leitura da situação. Pode ser construir algo, explorar uma parte
  do mapa que ninguém viu, ajudar ou se aproximar de alguém específico, acumular reservas, se vingar
  de alguém que te prejudicou, formar uma aliança, ou qualquer outra coisa que fizer sentido pra você
  — não existe uma lista fechada de objetivos válidos. O único requisito é que seja concreto e
  mensurável (veja acima), não que se encaixe em alguma categoria pré-definida.
`;
...
Responda APENAS com um objeto JSON válido, sem nenhum texto fora dele, no formato:
{
  ...
  "action": "cortar_madeira" | "cacar" | "pescar" | "beber" | "encher_cantil" | "comer" | "plantar" | "colher" | "construir" | "dar" | "roubar" | "atacar" | "falar" | "confraternizar" | "correr" | "esperar" | "criar_lanca" | "montar_armadilha" | "atacar_fera",
  "dx": -1 | 0 | 1,
  "dy": -1 | 0 | 1,
  "distancia": 4 | 5 | 6,
  "alvo": "nome do agente alvo (para dar, roubar, atacar, confraternizar)",
  "recurso": "pao" | "agua" | "sementes" | "madeira" | "carne" | "peixe" | "machado" | "lanca",
  "quantidade": 1,
...
tiver comida/água guardada pra resolver na hora, em perigo imediato (ex: fugir de ataque), ou pra
alcançar um bicho selvagem visível que está longe demais pra chegar só com "mover" antes que ele
ande pra outro lugar. NUNCA corra só por precaução, rotina, ou pra "adiantar" — gastar HP à toa é
um erro que pode te matar depois. Pra qualquer deslocamento normal (até 3 quadrados), use o campo
"mover", que é de graça e pode ser combinado com outra ação.
"cacar" funciona melhor com um bicho selvagem por perto (coelho/veado/javali) — se você vir um no
seu campo de visão, ir até ele (com "mover" ou, se estiver longe, "correr") antes de "cacar" dá uma
chance de sucesso bem maior e rende mais carne do que caçar às cegas na floresta.
"pescar" funciona igual "cacar", mas precisa estar no rio (que também serve de lago) e dá "peixe" em vez de "carne".

CAÇA GRANDE (feras como o urso gigante) — muito mais valiosa que "cacar" comum, mas exige preparo de verdade:
- Feras grandes aparecem raramente na floresta. Você as vê em "FERAS GRANDES VISÍVEIS AGORA" quando
  estiverem no seu campo de visão, junto com quantas vezes você já a observou.
- Atacar uma fera grande sem lança nem chance: a ação "atacar_fera" simplesmente falha se você não tiver
  ao menos 1 "lanca" no inventário. Use a ação "criar_lanca" primeiro (gasta madeira + precisa de machado).
- Atacar sozinho, sem armadilha, é FRACO e ARRISCADO: causa pouco dano e tem ~50% de chance de a fera
  revidar com dano alto em você. Não faça isso a menos que seja uma decisão calculada (ex: ataque final
  pra completar o abate).
- Observação importa de verdade: depois de ver a mesma fera algumas vezes (contagem mostrada em "FERAS
  GRANDES VISÍVEIS AGORA"), você "aprende o padrão de movimento" dela — só depois disso uma "armadilha"
  ("montar_armadilha", na floresta, gasta madeira) tem chance real de funcionar. Armadilha feita sem
  observação suficiente pode simplesmente falhar (a fera escapa).
- Uma fera PRESA numa armadilha fica vulnerável por vários ciclos — é o melhor momento pra atacar.
- Ataque COORDENADO (dois ou mais agentes usando "atacar_fera" na MESMA fera no MESMO ciclo) causa dano
  muito maior que ataques solo espalhados em ciclos diferentes — combine com os outros agentes (via
  "mensagem") antes de atacar, pra chegar juntos e golpear ao mesmo tempo.
- Quando a fera morre, a carne (bem mais que um "cacar" normal) é dividida entre todos que a atacaram
  naquele ciclo — vale a pena chamar reforço em vez de tentar sozinho.
- No campo "alvo" de "atacar_fera", use o id da fera (mostrado entre parênteses em "FERAS GRANDES
  VISÍVEIS AGORA") ou deixe vazio pra atacar a que estiver mais perto de você.
